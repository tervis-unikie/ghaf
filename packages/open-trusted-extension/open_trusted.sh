...
/run/current-system/sw/bin/ssh -o StrictHostKeyChecking=no business-vm "run-waypipe chromium --enable-features=UseOzonePlatform --ozone-platform=wayland \"${URL}\"" >/dev/null 2>&1
RES=$?
Msg "{\"status\":\"${RES}\"}"
